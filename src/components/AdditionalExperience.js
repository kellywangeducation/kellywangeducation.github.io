  ...
  return `
    <section id="additional-experience" class="py-12 md:py-20 bg-transparent">
      <div class="section-container">
        ...
        <div class="space-y-6">
          ${items.map((item, i) => `
            <div class="glass-panel p-8 rounded-3xl hover:bg-white/90 dark:hover:bg-neutral-900/90 transition-all duration-300 border border-white/50 dark:border-white/10 hover:shadow-lg scroll-fade group flex flex-row gap-4 md:gap-6 items-start md:items-center" style="transition-delay: ${i * 50}ms">
              <div class="p-3 md:p-4 bg-brand-surface dark:bg-black rounded-2xl shrink-0 group-hover:rotate-6 transition-transform duration-300">
                <i data-lucide="star" class="w-6 h-6 md:w-8 md:h-8 text-accent-pink"></i>
              </div>
              
              <div class="flex-grow">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-2">
                    <h3 class="text-xl md:text-2xl font-bold group-hover:text-accent-pink transition-colors">${item.role}</h3>
